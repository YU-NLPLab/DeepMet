...
if __name__ == '__main__':
    ...
    parser.add_argument("--alpha", default=0.2, type=float, required=False)
    ...
    test = pd.read_csv('./data/TOEFI/TOEFI_verb_features.csv')
    test2 = pd.read_csv('./data/TOEFI/TOEFI_allpos_features.csv')
    ...
    best_pred = (np.array(pred)/N_FOLD >= ALPHA).astype('int')
    best_pred2 = (np.array(pred)/N_FOLD >= ALPHA).astype('int')
    test['predict'] = best_pred
    test2['predict'] = best_pred2
    print(test.predict.value_counts())
    print(test2.predict.value_counts())
    test[['id', 'sentence', 'word', 'label', 'predict']].to_csv('./predict/TOEFI_verb_predict.csv', index=False)
    test2[['id', 'sentence', 'word', 'label', 'predict']].to_csv('./predict/TOEFI_allpos_predict.csv', index=False)
